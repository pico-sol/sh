...
# Here's an example using the name "halt_700" for the screen session:
# $ screen -S halt_700
# You're now inside the new screen session.

# 5)  Inside the screen session run the script:
# $ ./halt_validator_at_epoch.sh 700
...
if [[ -z $1 ]]; then
  echo "Usage: $0 <epoch>"
  echo "Waits until the specified epoch, then stops the validator."
  exit 0
fi
